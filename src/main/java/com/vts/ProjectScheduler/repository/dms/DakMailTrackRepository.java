...
public interface DakMailTrackRepository extends JpaRepository<DakMailTracking, Long> {
    ...
    @Query(value="SELECT m.DakId,m.EmpId,a.DakNo,sourceData.SourceShortName,a.ActionDueDate FROM dak_marking m JOIN dak a ON m.DakId=a.DakId LEFT JOIN dak_source_details sourceData ON sourceData.SourceDetailId=a.SourceDetailId WHERE a.ReceiptDate IS NOT NULL AND DATE(a.ReceiptDate) >='2024-04-01' AND m.DakId NOT IN (SELECT r.DakId FROM dak_reply r WHERE r.DakId=m.DakId) AND a.DakStatus IN('DD','DA') AND a.ActionId='2' AND m.IsActive=1",nativeQuery = true)
    public List<Object[]> getDailyPendingReplyEmpData();

    @Query(value="SELECT m.DakId,m.EmpId,a.DakNo,sourceData.SourceShortName,a.ActionDueDate FROM dak_marking m INNER JOIN dak a ON m.DakId=a.DakId AND DATE(a.DistributedDate) = CURDATE() AND  a.DakStatus != 'DI' LEFT  JOIN dak_source_details sourceData ON sourceData.SourceDetailId=a.SourceDetailId WHERE m.IsActive=1",nativeQuery = true)
